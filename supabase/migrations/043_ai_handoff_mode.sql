...
--    de contestar. El default se siembra en español neutro; la cuenta lo
--    edita en Ajustes → IA, y si lo deja vacío no se envía nada
--    (comportamiento actual, elegido a propósito). Este mensaje NO
--    consume cupo de respuesta (`claim_ai_reply_slot`) ni cuenta como
--    respuesta de IA: es un acuse, no una respuesta.
--
...
ALTER TABLE public.ai_configs
  ADD COLUMN IF NOT EXISTS handoff_message text
    DEFAULT 'Gracias por escribirnos. Un miembro de nuestro equipo continuará la conversación en breve.';

...
// same body replays the stored response instead of sending a second
// message (fase 7 §1). See `src/lib/api/v1/idempotency.ts`.
//
// Response (201):
...
export async function POST(request: Request) {
  try {
    ...
    // `withIdempotency` owns the body: it enforces `Content-Type`, the
    // 1 MiB ceiling and "must be a JSON object" through `readJsonBody`,
    // hashes the raw bytes, and — when the caller sent an
    // `Idempotency-Key` — replays a previous response instead of
    // running any of this a second time.

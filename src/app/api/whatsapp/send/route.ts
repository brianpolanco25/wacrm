...
export async function POST(request: Request) {
  try {
    ...
    const {
      ...
      interactive_payload,
      reply_to_message_id,
      // Fase 4 §1: explicit sender number. The inbox never sends it —
      // the conversation already carries the number the customer wrote
      // to. The "contact → send template" path does, because there is
      // no thread to read it from yet.
      whatsapp_config_id,
    } = body;

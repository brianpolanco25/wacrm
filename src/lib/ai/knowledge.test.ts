...
describe('embeddings provider (068)', () => {
  it('ingest passes the provider to embedTexts', async () => {
    const { db } = makeDb();
    await ingestDocument(
      db,
      'acct',
      { embeddingsApiKey: 'AIza-x', embeddingsProvider: 'gemini' },
      'doc-1',
      'hello world'
    );
  ...
  it('retrieve passes the provider to embedTexts, and defaults to openai', async () => {
    const { db, state } = makeDb();
    state.semantic = [{ id: 'c1', content: 'x' }];
    ...
    expect(h.embedTexts).toHaveBeenLastCalledWith('AIza-x', ['q'], 'gemini');
    await retrieveKnowledge(db, 'acct', { embeddingsApiKey: 'sk-x' }, 'q');
    expect(h.embedTexts).toHaveBeenLastCalledWith('sk-x', ['q'], 'openai');
  });
});
